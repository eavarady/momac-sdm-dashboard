...
from sdm_bottlenecks.bottleneck_detector import detect_bottleneck


def _df_production_log(rows):
    return pd.DataFrame(rows, columns=["timestamp", "line_id", "product_id", "step_id", "quantity", "status"])


def _df_process_steps(rows):
    # not used by v1 heuristic, but keep shape realistic
    return pd.DataFrame(rows, columns=["product_id", "step_id", "step_name", "assigned_machine", "assigned_operators", "estimated_time", "dependency_step_id"])
