...
def test_read_excel_tables_happy_path(tmp_path):
    ...
    with pd.ExcelWriter(xlsx, engine="openpyxl") as w:
        pd.DataFrame(
            {"machine_id": ["M1"], "line_id": ["L1"], "type": ["robot"], "status": ["online"]}
        ...
        pd.DataFrame(
            {"product_id": ["P1"], "name": ["Prod"], "category": [""], "spec_version": [""]}
        ...
        pd.DataFrame(
            {"timestamp": ["2025-09-01T10:00:00Z"], "machine_id": ["M1"], "metric_type": ["temp"], "metric_value": ["42.0"]}
        ).to_excel(w, sheet_name="machine_metrics", index=False)

        pd.DataFrame(
            {"timestamp": ["2025-09-01T10:00:00Z"], "product_id": ["P1"], "check_type": ["dim"], "result": ["pass"], "inspector_id": ["O1"]}
